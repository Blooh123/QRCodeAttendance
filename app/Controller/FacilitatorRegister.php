...
class FacilitatorRegister extends \Controller{
    ...
    public function validateAccount(){
        ...
        if (empty($username) || empty($password)) {
            ...
            $query = "SELECT id, username, pass, roles FROM user_account WHERE username = :username";
            ...
            $query = "SELECT id, username, pass, roles FROM user_account WHERE username = :username AND pass = SHA2(:password, 256)";
            $stmt = $pdo->prepare($query);
            $stmt->bindParam(':username', $username);
